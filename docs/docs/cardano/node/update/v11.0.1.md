...
<Tabs groupId="node" queryString="node">
  ...
  wget --no-use-server-timestamps -q \
    https://raw.githubusercontent.com/IntersectMBO/cardano-node/10.7.1/configuration/cardano/mainnet-config.json \
    -O $NODE_HOME/${NODE_CONFIG}-config.json
  ```

  ```bash
  wget --no-use-server-timestamps -q \
    https://raw.githubusercontent.com/IntersectMBO/cardano-node/10.7.1/configuration/cardano/mainnet-checkpoints.json \
